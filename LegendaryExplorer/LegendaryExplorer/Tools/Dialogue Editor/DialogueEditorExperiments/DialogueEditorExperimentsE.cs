...
namespace LegendaryExplorer.DialogueEditor.DialogueEditorExperiments
{
    ...
    static class DialogueEditorExperimentsE
    {
        ...
        /// <summary>
        /// Clones a Dialogue Node and its related Sequence, while letting the user provide a unique id
        /// or request an auto-generated one.
        /// </summary>
        ...
        public static void CloneNodeAndSequence(DialogueEditorWindow dew)
        {
            ...
            {
                HashSet<int> usedIDs = new(dew.SelectedConv.EntryList.Concat(dew.SelectedConv.ReplyList)
                    .Select(node => node.ExportID)
                    .Where(id => id > 0));

                int autoGeneratedID = usedIDs.Any() ? usedIDs.Max() + 1 : 1;
                string promptMessage = "Enter a new unique ExportID (positive integer), or leave blank to auto-generate one.";
                string promptDefault = "";

                string idPrompt = PromptDialog.Prompt(null,
                    promptMessage,
                    "Clone Node and Sequence",
                    promptDefault,
                    validator: input =>
                    {
                        if (string.IsNullOrWhiteSpace(input))
                        {
                            return (true, "Leave blank to auto-generate a unique ExportID.");
                        }

                        if (!int.TryParse(input, out int parsedID) || parsedID < 1)
                        {
                            return (false, "ExportID must be a positive integer, or leave blank for auto-generate.");
                        }

                        if (usedIDs.Contains(parsedID))
                        {
                            return (false, "That ExportID is already in use in this conversation.");
                        }
                ...
                int newID = string.IsNullOrWhiteSpace(idPrompt) ? autoGeneratedID : int.Parse(idPrompt);
